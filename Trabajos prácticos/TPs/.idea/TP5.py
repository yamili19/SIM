...
class vectDatos:
    def __init__(self, evento, reloj,
                 ...
                 HoraFin_Detec1, HoraFin_Detec2, HoraFin_Detec3, HoraFin_Detec4, llegadaVirus, horaVirus, rndQueDetiene,
                 queDetiene, finVirus, horaFinVirus,
                 ColaEstacionamiento,
        ...
        #llegada virus y que detiene (servidor o llegada cliente)
        self.llegadaVirus = llegadaVirus
        self.horaVirus = horaVirus
        self.rndQueDetiene = rndQueDetiene
        self.queDetiene = queDetiene

        #fin virus
        self.finVirus = finVirus
        self.horaFinVirus = horaFinVirus
...
def menorTiempoEventos(vecEstado, i):
    x = [vecEstado[i].horaLL_llegVehiculo, vecEstado[i].HoraLL_llgrupo, vecEstado[i].horaFin_fCobroEstacionamiento1,
         ...
         vecEstado[i].HoraFin_Detec2, vecEstado[i].HoraFin_Detec3, vecEstado[i].HoraFin_Detec4, vecEstado[i].horaVirus,
         vecEstado[i].horaFinVirus]
...
def tipoEvento(vecEstado, i, relojActual):
    ...
    elif relojActual == vecEstado[i].horaVirus:
        return "llegada virus"
    elif relojActual == vecEstado[i].horaFinVirus:
        return "fin virus"
    else:
        return "WTF"
...
def detieneServidorLlegada(rnd):
    if rnd < 0.35:
        resultado = "LlegadasVehiculos"
    else:
        resultado = "ServidorDetector4"
    return resultado

def cuandoSeDeteniene(A, h, ultimaFila):
    K1, K2, K3, K4 = [], [], [], []
    tAnt = []
    tSig = []
    AAnt = []
    ...
    b = []
    func = []
    beta = float("{:.2f}".format(random.random()))
    if A != 0:
        ASiguiente = A
        while ASiguiente <= 3*Ainicial:
            func.append("A*Beta")
            b.append(beta)
            k1 = beta*A
            k2 = beta*(A+h/2*k1)
            ...
            K3.append(k3)
            K4.append(k4)
            tAnt.append(tSiguiente)
            AAnt.append(ASiguiente)
            tSiguiente = tSiguiente + h
            tSig.append(tSiguiente)
            ASiguiente = ASiguiente+h/6*(k1+2*k2+2*k3+k4)
            ASig.append(ASiguiente)
        data = {
            'Funcion':func,'Beta':b,'t': tAnt, 'A': AAnt, 'k1':K1, 'K2':K2, 'K3':K3, 'K4':K4, 't+1':tSig, 'A+1':ASig
        }

        ultimaFila.append({
            'Funcion': func[0], 'Beta': b[0], 't': tAnt[0], 'A': AAnt[0], 'k1': K1[0], 'K2': K2[0], 'K3': K3[0],
            'K4': K4[0], 't+1': tSig[0], 'A+1': ASig[0]
        })

        ultimaFila.append({
            'Funcion':func[-1],'Beta':b[-1],'t': tAnt[-1], 'A': AAnt[-1], 'k1':K1[-1], 'K2':K2[-1], 'K3':K3[-1],
            'K4':K4[-1], 't+1':tSig[-1], 'A+1':ASig[-1]
        })
        ultimaFila.append({
            'Funcion': '', 'Beta': '', 't': '', 'A': '', 'k1': '', 'K2': '', 'K3': '',
            'K4': '', 't+1': '', 'A+1': ''
        })

    return tSiguiente, data, ultimaFila
#= - (L / (0.8*t2)) - L

def detencionLlegadas(reloj, h, ultimaFila):
    LSiguiente = reloj
    tSiguiente = h
    LAnterior = 0
    K1, K2, K3, K4 = [], [], [], []
    ...
    tAnt = []
    func = []
    while (LSiguiente - LAnterior) >= 1 or LAnterior == 0:
        func.append("-(L/(0.8*t2))-L")
        k1 = - (LSiguiente / (0.8*tSiguiente**2)) - LSiguiente
        k2 = - ((LSiguiente+h/2*k1) / (0.8*(tSiguiente+h/2)**2)) - (LSiguiente+h/2*k1)
        k3 = - ((LSiguiente+h/2*k2) / (0.8*(tSiguiente+h/2)**2)) - (LSiguiente+h/2*k2)
        k4 = - ((LSiguiente+h*k3) / (0.8*(tSiguiente+h)**2)) - (LSiguiente+h*k3)
        K1.append(k1)
        K2.append(k2)
        K3.append(k3)
        K4.append(k4)
        LAnterior = LSiguiente
        LAnt.append(LAnterior)
        tAnt.append(tSiguiente)
        tSiguiente = tSiguiente + h
        tSig.append(tSiguiente)
        LSiguiente = LSiguiente+h/6*(k1+2*k2+2*k3+k4)
        LSig.append(LSiguiente)
    data = {
        'Funcion':func, 't':tAnt, 'L':LAnt, 'k1':K1, 'k2':K2, 'k3':K3, 'k4':K4, 't+1':tSig, 'L+1':LSig
    }

    ultimaFila.append({
        'Funcion': func[0], 't': tAnt[0], 'L': LAnt[0], 'k1': K1[0], 'k2': K2[0], 'k3': K3[0], 'k4': K4[0],
        't+1': tSig[0], 'L+1': LSig[0]
    })

    ultimaFila.append({
        'Funcion': func[-1], 't': tAnt[-1], 'L': LAnt[-1], 'k1': K1[-1], 'k2': K2[-1], 'k3': K3[-1], 'k4': K4[-1],
        't+1': tSig[-1], 'L+1': LSig[-1]
    })

    ultimaFila.append({
        'Funcion': '', 't': '', 'L': '', 'k1': '', 'k2': '', 'k3': '', 'k4': '', 't+1': '', 'L+1': ''
    })

    return tSiguiente, data, ultimaFila
#DS / dt = (0.2 * S) + 3 - t
def detencionServidor(reloj, h, ultima):
    SSiguiente = reloj
    tSiguiente = 0
    K1, K2, K3, K4 = [], [], [], []
    LSig = []
    ...
    tAnt = []
    func = []
    while SSiguiente <= reloj+reloj*0.50:
        func.append("(0.2*S)+3-t")
        k1 = (0.2 * SSiguiente) + 3 - tSiguiente
        k2 = (0.2 * (SSiguiente+h/2*k1)) + 3 - (tSiguiente+h/2)
        k3 = (0.2 * (SSiguiente+h/2*k2)) + 3 - (tSiguiente+h/2)
        k4 = (0.2 * (SSiguiente+h*k3)) + 3 - (tSiguiente+h)
        K1.append(k1)
        K2.append(k2)
        K3.append(k3)
        K4.append(k4)
        LAnt.append(SSiguiente)
        tAnt.append(tSiguiente)
        tSiguiente = tSiguiente + h
        tSig.append(tSiguiente)
        SSiguiente = SSiguiente + h/6*(k1+2*k2+2*k3+k4)
        LSig.append(SSiguiente)
    data = {
        'Funcion':func,'t': tAnt, 'S': LAnt, 'k1': K1, 'k2': K2, 'k3': K3, 'k4': K4, 't+1': tSig, 'S+1': LSig
    }

    ultima.append({
        'Funcion': func[0], 't': tAnt[0], 'S': LAnt[0], 'k1': K1[0], 'k2': K2[0], 'k3': K3[0], 'k4': K4[0],
        't+1': tSig[0], 'S+1': LSig[0]
    })

    ultima.append({
        'Funcion': func[-1], 't': tAnt[-1], 'S': LAnt[-1], 'k1': K1[-1], 'k2': K2[-1], 'k3': K3[-1], 'k4': K4[-1],
        't+1': tSig[-1], 'S+1': LSig[-1]
    })

    ultima.append({
        'Funcion': '', 't': '', 'S': '', 'k1': '', 'k2': '', 'k3': '', 'k4': '', 't+1': '', 'S+1': ''
    })

    return tSiguiente, data, ultima

def llegadaDetencion(vectorEstado, i, h, data1, data2, ultimaFila1, ultimaFila2, tS, tL):
    vectorEstado[i].rndQueDetiene = float("{:.2f}".format(random.random()))
    vectorEstado[i].queDetiene = detieneServidorLlegada(vectorEstado[i].rndQueDetiene)
    if vectorEstado[i].queDetiene == "ServidorDetector4":
        t, data1, ultimaFila1 = detencionServidor(vectorEstado[i].reloj, h, ultimaFila1)
        vectorEstado[i].finVirus = tS*t
        vectorEstado[i].horaFinVirus = vectorEstado[i].reloj + vectorEstado[i].finVirus
        if vectorEstado[i].Detector4 == "Ocupado":
            vectorEstado[i].HoraFin_Detec4 += vectorEstado[i].finVirus  # 0 mal -> seria un recalculo de tiempo puede ser la hora fin + tiempo de duracion de la detencion
            #vectorEstado[i].ColaDetector += 1 #puede haber error aca 
            #faltaria cambiarle  el  estado a la persona a Detenido
            for lpersona in range(len(vectorEstado[i].oPersona)):
                if vectorEstado[i].oPersona[lpersona].estado == "EnDeteccion4":
                    vectorEstado[i].oPersona[lpersona].estado = "Detenido"
                    break
        vectorEstado[i].Detector4 = "Detenido"
        vectorEstado[i].horaVirus = 0
    else:
        t, data2, ultimaFila2 = detencionLlegadas(vectorEstado[i].reloj, h, ultimaFila2)
        vectorEstado[i].finVirus = tL*t
        vectorEstado[i].horaFinVirus = vectorEstado[i].reloj + vectorEstado[i].finVirus
        vectorEstado[i].horaLL_llegVehiculo += vectorEstado[i].finVirus #mal -> seria un recalculo de tiempo puede ser la hora fin + tiempo de duracion de la detencion
        vectorEstado[i].horaVirus = 0
    return vectorEstado, data1, data2, ultimaFila1, ultimaFila2

def finDetencion(vectorEstado, i, h, A, tA, ultimaFila):
    if vectorEstado[i].queDetiene == "ServidorDetector4":
        band = False
        #verificar que haya una persona en estado Detenido
        for lpersona in range(len(vectorEstado[i].oPersona)):
            if vectorEstado[i].oPersona[lpersona].estado == "Detenido":
                vectorEstado[i].oPersona[lpersona].estado = "EnDeteccion4"
                band = True
                #vectorEstado[i].ColaDetector -= 1
                break
        
        if not band:
            if vectorEstado[i].ColaDetector > 0:
                #verificamos q haya alguna persona esperando en la fila
                for lpersona in range(len(vectorEstado[i].oPersona)):
                    #buscamos a la persona y le cambiamos el estado a EnDeteccion
                    if vectorEstado[i].oPersona[lpersona].estado == "EnColaDeteccion":
                        vectorEstado[i].oPersona[lpersona].estado = "EnDeteccion4"
                        vectorEstado[i].ColaDetector -= 1
                        vectorEstado[i].Detector4 = "Ocupado"
                        break
                vectorEstado[i].HoraFin_Detec4 = vectorEstado[i].reloj + d.finDeteccion 
            else:
                vectorEstado[i].Detector4 = "Libre"
    #recalculo de  la nueva llegada
    t, proxDetencion, ultimaFila = cuandoSeDeteniene(A, h, ultimaFila)
    vectorEstado[i].llegadaVirus = t * tA
    #vecEstado[j].horaVirus = vecEstado[j].reloj + vecEstado[j].llegadaVirus
    vectorEstado[i].horaVirus = vectorEstado[i].llegadaVirus + vectorEstado[i].reloj #aca calcula la nueva llegada  del virus, pero tendria q ser diferente
    #
    vectorEstado[i].horaFinVirus = 0 #aca iria el recalculo de la nueva hora de llegada del virus
    return vectorEstado, proxDetencion, ultimaFila



def FuncionPrincipal(n, ver, mediallv, mediallgrupoSinAuto, mediaFinCobroEstacionamiento, mediafinCobroEntrada,
                 finControlEntradas, finDeteccion, h, tA, tL, tS):
    global d
    ...
    for i in range(numero):
        ...
        # logica para la fila = 1
            ...
            if evento == 0:
                vecEstado = EventoLLegadaVehiculo(vecEstado,1)  # TODO: definir que posicion del vector ocupa si [0] o [1]
                #Contamos la cantidad de llegadas para calcular la llegada de la interrupcion del virus
                cantLlegadas += 1
            # llegada de un nuevo grupo --> genera una nueva llegada
                ...
                vecEstado, data1, data2, ultimaFila1, ultimaFila2= llegadaDetencion(vecEstado, 1, h, data1, data2, ultimaFila1, ultimaFila2, tS, tL)
            elif evento == 21:
                vecEstado, proxDetencion, ultimaFila = finDetencion(vecEstado, 1, h, A, tA, ultimaFila)
            # para i = 0
        elif (i % 2 == 0):
            j = 0
            ...
            if evento == 0:
                ...
                cantLlegadas += 1 # Esto lo hago para contar las 150 llegadas y así poder hacer el calculo de cuando va a venir el virus
            # llegada de un nuevo grupo --> genera una nueva llegada
            elif evento == 1:
                ...
                vecEstado, data1, data2, ultimaFila1, ultimaFila2 = llegadaDetencion(vecEstado, 0, h, data1, data2, ultimaFila1, ultimaFila2, tS, tL)
            elif evento == 21:
                vecEstado, proxDetencion, ultimaFila = finDetencion(vecEstado, 0, h, A, tA, ultimaFila)
        if cantLlegadas == 150:
            if A == 0:
                A = vecEstado[j].reloj  # A es la constante que se utiliza en la ecuacion diferencial que se usa para
                # determinar cuando va a venir el virus (conte todas las llegadas porque entendio que es con todas las
                # llegadas y no solo con todas las que vas a detener por culpa del virus)
                t, proxDetencion, ultimaFila = cuandoSeDeteniene(A, h, ultimaFila)
                vecEstado[j].llegadaVirus = t * tA
                vecEstado[j].horaVirus = vecEstado[j].reloj + vecEstado[j].llegadaVirus
        if (i >= ver-1 and i <= ver-1 + 500) or (i == n-1):
            gca = ""
            gsa = ""
            p = ""
            v = ""
            ...
            oPersona.append(p)
            oVehiculo.append(v)
            llegVirus.append(vecEstado[j].llegadaVirus)
            horaVirus.append(vecEstado[j].horaVirus)
            finVirus.append(vecEstado[j].finVirus)
            horaFinVirus.append(vecEstado[j].horaFinVirus)
            rndDetiene.append(vecEstado[j].rndQueDetiene)
            detiene.append(vecEstado[j].queDetiene)
    ...
    data = {
        ...
        'tiempoLlegadaVirus': llegVirus,
        'horaLlegadaVirus': horaVirus,
        'rndQueDetiene': rndDetiene,
        'queDetiene': detiene,
        'tiempoFinVirus': finVirus,
        'horaFinVirus': horaFinVirus,
    ...
    df1 = pd.DataFrame(proxDetencion)

    df2 = pd.DataFrame(data1)
    df3 = pd.DataFrame(data2)
    df4 = pd.DataFrame(ultimaFila)
    df5 = pd.DataFrame(ultimaFila1)
    df6 = pd.DataFrame(ultimaFila2)
    workbook = Workbook()

    hoja_activa = workbook.active

    hojaRKCuandoLlegaVirus = workbook.create_sheet(title="CuandoVirus")
    hojaRKDetencionServidor = workbook.create_sheet(title="DetencionServidor")
    hojaRKDetencionLlegadas = workbook.create_sheet(title="DetecionLlegadas")
    ...
    df1[''] = ''
    df4[''] = ''
    df_combined = pd.concat([df1, pd.DataFrame(columns=['']), df4], axis=1)
    filaRKCuandoLlegaVirus = dataframe_to_rows(df_combined, index=False, header=False)

    encabezado = list(df_combined.columns)
    hojaRKCuandoLlegaVirus.append(encabezado)
    hojaRKCuandoLlegaVirus.freeze_panes = "A2"

    for f in filaRKCuandoLlegaVirus:
        hojaRKCuandoLlegaVirus.append(f)

    df2[''] = ''
    df5[''] = ''
    df_combined = pd.concat([df2, pd.DataFrame(columns=['']), df5], axis=1)
    filaRKDetencionServidor = dataframe_to_rows(df_combined, index=False, header=False)

    encabezado = list(df_combined.columns)
    hojaRKDetencionServidor.append(encabezado)
    hojaRKDetencionServidor.freeze_panes = "A2"

    for ff in filaRKDetencionServidor:
        hojaRKDetencionServidor.append(ff)

    df3[''] = ''
    df6[''] = ''
    df_combined = pd.concat([df3, pd.DataFrame(columns=['']), df6], axis=1)
    filaRKDetencionLlegadas = dataframe_to_rows(df_combined, index=False, header=False)

    encabezado = list(df_combined.columns)
    hojaRKDetencionLlegadas.append(encabezado)
    hojaRKDetencionLlegadas.freeze_panes = "A2"

    for fff in filaRKDetencionLlegadas:
        hojaRKDetencionLlegadas.append(fff)
...
class colasApp:
    def __init__(self, master):
        ...
        self.h_frame = customtkinter.CTkFrame(master=master)
        self.h_frame.grid(row=0, column=2, padx=10, pady=10, sticky="nw")
        self.h_lbl = customtkinter.CTkLabel(master=self.h_frame, text="h")
        self.h_lbl.grid(row=0, column=0, padx=10, pady=0, sticky="s")
        self.h_var = customtkinter.StringVar()
        self.h_var.set("0.1")
        self.h_entry = customtkinter.CTkEntry(self.h_frame, textvariable=self.h_var)
        self.h_entry.grid(row=1, column=0, padx=5, pady=5)

        self.tA_frame = customtkinter.CTkFrame(master=master)
        self.tA_frame.grid(row=3, column=0, padx=10, pady=10, sticky="nw")
        self.tA_lbl = customtkinter.CTkLabel(master=self.tA_frame,
                                                          text="¿Cuándo actúa el virus?\nt=1")
        self.tA_lbl.grid(row=0, column=0, padx=10, pady=0, sticky="s")
        self.tA_var = customtkinter.StringVar()
        self.tA_var.set("30")
        self.tA_var.set("10")
        self.tA_entry = customtkinter.CTkEntry(self.tA_frame,
                                                            textvariable=self.tA_var)
        self.tA_entry.grid(row=1, column=0, padx=5, pady=5)

        self.tS_frame = customtkinter.CTkFrame(master=master)
        self.tS_frame.grid(row=3, column=1, padx=10, pady=10, sticky="nw")
        self.tS_lbl = customtkinter.CTkLabel(master=self.tS_frame,
                                             text="¿Cuánto se detiene el servidor?\nt=1")
        self.tS_lbl.grid(row=0, column=0, padx=10, pady=0, sticky="s")
        self.tS_var = customtkinter.StringVar()
        self.tS_var.set("8")
        self.tS_entry = customtkinter.CTkEntry(self.tS_frame,
                                               textvariable=self.tS_var)
        self.tS_entry.grid(row=1, column=0, padx=5, pady=5)

        self.tL_frame = customtkinter.CTkFrame(master=master)
        self.tL_frame.grid(row=3, column=2, padx=10, pady=10, sticky="nw")
        self.tL_lbl = customtkinter.CTkLabel(master=self.tL_frame,
                                             text="¿Cuánto se detienen las llegadas?\nt=1")
        self.tL_lbl.grid(row=0, column=0, padx=10, pady=0, sticky="s")
        self.tL_var = customtkinter.StringVar()
        self.tL_var.set("27")
        self.tL_entry = customtkinter.CTkEntry(self.tL_frame,
                                               textvariable=self.tL_var)
        self.tL_entry.grid(row=1, column=0, padx=5, pady=5)

        self.conclu_frame = customtkinter.CTkFrame(master=master)
        self.conclu_frame.grid(row=4, column=0, columnspan=3, padx=10, pady=10, sticky="news")

        self.conclu_metrics = customtkinter.CTkLabel(master=self.conclu_frame, text="Métricas")
        self.conclu_metrics.grid(row=4, column=0, padx=10, pady=0, sticky="swe")

        self.conclu_lbl1 = customtkinter.CTkLabel(master=self.conclu_frame, text="")
        self.conclu_lbl1.grid(row=5, column=0, padx=10, pady=0, sticky="sw")
        self.conclu_lbl2 = customtkinter.CTkLabel(master=self.conclu_frame, text="")
        self.conclu_lbl2.grid(row=6, column=0, padx=10, pady=0, sticky="sw")
        self.conclu_lbl3 = customtkinter.CTkLabel(master=self.conclu_frame, text="")
        self.conclu_lbl3.grid(row=7, column=0, padx=10, pady=0, sticky="sw")
        self.conclu_lbl4 = customtkinter.CTkLabel(master=self.conclu_frame, text="")
        self.conclu_lbl4.grid(row=8, column=0, padx=10, pady=0, sticky="sw")
    ...
    def principal(self):
        vector = FuncionPrincipal(int(self.cantidad_numeros_var.get()), int(self.ver_desde_var.get()),
                                  ...
                                  float(self.h_var.get()), float(self.tA_var.get()), float(self.tL_var.get()),
                                  float(self.tS_var.get()))
